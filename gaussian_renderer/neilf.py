...
def render_view(
    ...
    direct_light_env_light = dict_params.get("env_light")  # 直接光照：环境光
    ...
    rotations = None
    cov3D_precomp = None
    if pipe.compute_cov3D_python:  # 预计算3D协方差
    ...
    shs = None
    colors_precomp = None  # 预计算SH颜色
    if override_color is None:
        if pipe.compute_SHs_python:
            dir_pp_normalized = F.normalize(
                viewpoint_camera.camera_center.repeat(means3D.shape[0], 1) - means3D,
                dim=-1,
            )
    ...
    # NOTE - incidents代表逐高斯元的间接光照（SH表示）
    ...
    if is_training:
        brdf_color, extra_results = rendering_equation(  # NOTE - PBR渲染方程
            base_color,
            roughness,
            normal.detach(),
            viewdirs,
        ...
        for i in range(0, means3D.shape[0], chunk_size):
            _brdf_color, _extra_results = rendering_equation(
                base_color[i : i + chunk_size],
                roughness[i : i + chunk_size],
                normal[i : i + chunk_size].detach(),
                viewdirs[i : i + chunk_size],
                incidents[i : i + chunk_size],
                direct_light_env_light,
        ...
        extra_results = {
            k: torch.cat([_extra_results[k] for _extra_results in extra_results], dim=0)
            for k in extra_results[0]
        }
    ...
    depths = (xyz_homo @ viewpoint_camera.world_view_transform)[
        :, 2:3
    ]  # 高斯元在相机坐标系下的z坐标，透视除法之前
    depths2 = depths.square()

    if is_training:
        features = torch.cat(  # NOTE - Feature
            [
                depths,
                depths2,
                brdf_color,
    ...
    (
        ...
        features=features,  # REVIEW - 注意观察一下Feature在GPU管线中的变化，前馈和反馈
    )
    ...
    if is_training:
        (
            ...
            rendered_diffuse,
            rendered_visibility,
        ) = rendered_feature.split(
            [1, 1, 3, 3, 3, 1, 3, 1], dim=0
        )  # 解包Feature
        feature_dict.update(
            {
...
def calculate_loss(viewpoint_camera, pc, results, opt, direct_light_env_light):
    ...
    if opt.lambda_light_smooth > 0:
        ...
        loss_light_smooth = first_order_edge_aware_loss(
            rendered_diffuse * image_mask, rendered_normal
        )
...
def render_neilf(
    ...
    **kwargs
    ...
    if is_training:
        loss, tb_dict = calculate_loss(  # 损失函数
            viewpoint_camera,
            pc,
            results,
            opt,
...
def rendering_equation(
    ...
    incident_visibility = visibility_precompute  # 阴影
    global_incident_lights = global_incident_lights * incident_visibility
    incident_lights = local_incident_lights + global_incident_lights  # 光照项

    n_d_i = (normals[:, None] * incident_dirs).sum(-1, keepdim=True).clamp(min=0)  # 余弦项
    f_d = base_color[:, None] / np.pi  # 漫反射项
    f_s = GGX_specular(
