...
def render_view(
    ...
    shs = None
    colors_precomp = None  # 预计算的SH颜色
    if override_color is None:
        if pipe.compute_SHs_python:
            shs_view = pc.get_shs.transpose(1, 2).view(
    ...
    dir_pp = pc.get_xyz - camera.camera_center.repeat(pc.get_shs.shape[0], 1)
    dir_pp_normalized = F.normalize(dir_pp, dim=-1)  # 摄像机指向高斯元的向量

    xyz_homo = torch.cat([means3D, torch.ones_like(means3D[:, :1])], dim=-1)  # 齐次坐标
    depths = (xyz_homo @ camera.world_view_transform)[:, 2:3]  # z深度，透视除法之前的深度
    depths2 = depths.square()
    features = torch.cat(
        [normals, depths, depths2], dim=-1
    )  # NOTE - Feature: normals, depths, depths2
    ...
    rendered_normal, rendered_depth, rendered_depth2 = torch.split(  # 解包Feature
        rendered_feature, [3, 1, 1], dim=0
    )

    rendered_var = rendered_depth2 - rendered_depth.square()  # 深度的方差
...
def calculate_loss(viewpoint_camera, pc, render_pkg, opt, iteration):
    ...
    Ll1 = F.l1_loss(rendered_image, gt_image)  # 3dgs经典L1损失
    ssim_val = ssim(rendered_image, gt_image)  # 3dgs SSIM损失
    ...
    if opt.lambda_normal_smooth > 0:  # 法线平滑
    ...
    if opt.lambda_depth_smooth > 0:  # 深度平滑
    ...
    if opt.lambda_point_entropy > 0:  # 点熵，鼓励每个高斯元地opacity接近0或者1
    ...
    if opt.lambda_orientation > 0 and iteration > opt.lambda_orientation_from_iter:
        ...
        directions = render_pkg["directions"]  # direction: 摄像机指向高斯元的向量
        loss_orientation = (
            ws * (normals * directions).sum(-1, keepdim=True).clamp_min(0.0)
        ).mean()  # 鼓励高斯元的正面法线朝向摄像机
        tb_dict["loss_orientation"] = loss_orientation.item()
        loss = loss + opt.lambda_orientation * loss_orientation

    if opt.lambda_depth_var > 0:  # 深度方差尽可能地小，高斯元要集中
        ...
        lambda_depth_var = opt.lambda_depth_var * min(
            math.pow(10, iteration / 5000), 100
        )
        # lambda_depth_var = opt.lambda_depth_var
        loss = loss + lambda_depth_var * loss_depth_var

    if opt.lambda_surface > 0:
        center, _ = torch.median(pc.get_xyz, dim=0)  # 一群高斯元的中位数中心
        loss_surface = torch.exp(-(pc.get_xyz - center[None, ...]).abs().mean())  # 远离中心

        tb_dict["loss_surface"] = loss_surface.item()
        loss = loss + opt.lambda_surface * loss_surface

    if opt.lambda_scaling > 0:  # 鼓励均匀缩放
        scaling = pc.get_scaling
        scaling_loss = (
            (scaling - scaling.mean(dim=-1, keepdim=True)).abs().sum(-1).mean()
        )
        lambda_scaling = opt.lambda_scaling - 0.99 * opt.lambda_scaling * min(
            1, 4 * iteration / opt.iterations
        )
...
def render(
    ...
    results = render_view(  # 推理
        ...
        override_color,
        computer_pseudo_normal=True
        if opt is not None and opt.lambda_normal_render_depth > 0
        else False,
    )
