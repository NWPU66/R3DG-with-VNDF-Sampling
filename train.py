...
def training(
    ...
    if args.checkpoint:  # 从checkpoint中恢复训练
    ...
    gaussians.training_setup(opt)  # 初始化参数，设置优化器
    ...
    if is_pbr:  # 前30000轮只训练几何，不训练材质
        ...
        if args.checkpoint:
            ...
            if os.path.exists(env_checkpoint):  # 这里是载入环境光贴图，不存在无所谓的
                direct_env_light.create_from_ckpt(
                    env_checkpoint, restore_optimizer=True
                )
    ...
    bg_color = [1, 1, 1] if dataset.white_background else [0, 0, 0]  # 只有黑白背景
    ...
    for iteration in progress_bar:
        ...
        render_pkg = render_fn(  # NOTE - 渲染函数
            # 从这里进入3dgs或neilf的管线
            viewpoint_cam,
            gaussians,
            pipe,
...
def eval_render(scene, gaussians, render_fn, pipe, background, opt, pbr_kwargs):
    ...
    progress_bar = tqdm(
        range(0, len(test_cameras)),
        desc="Evaluating",
        initial=0,
        total=len(test_cameras),
    )
    ...
    with torch.no_grad():
        for idx in progress_bar:
            ...
            save_image(
                image,
                os.path.join(
                    args.model_path, "eval", "render", f"{viewpoint.image_name}.png"
                ),
            )
            save_image(
                gt_image,
                os.path.join(
                    args.model_path, "eval", "gt", f"{viewpoint.image_name}.png"
                ),
            )
            save_image(
                results["normal"] * 0.5 + 0.5,
                os.path.join(
                    args.model_path, "eval", "normal", f"{viewpoint.image_name}.png"
                ),
            )
            if gaussians.use_pbr:
                save_image(
                ...
                save_image(
                    results["lights"],
                    os.path.join(
                        args.model_path, "eval", "lights", f"{viewpoint.image_name}.png"
                    ),
                )
                save_image(
                    results["local_lights"],
                    os.path.join(
                        args.model_path, "eval", "local", f"{viewpoint.image_name}.png"
                    ),
                )
                save_image(
                    results["global_lights"],
                    os.path.join(
                        args.model_path, "eval", "global", f"{viewpoint.image_name}.png"
                    ),
                )
                save_image(
                    results["visibility"],
    ...
    print(
        "\n[ITER {}] Evaluating {}: PSNR {} SSIM {} LPIPS {}".format(
            args.iterations, "test", psnr_test, ssim_test, lpips_test
        )
    )
